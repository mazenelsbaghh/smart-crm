...
async def test_emergency_stop_is_idempotent_and_requires_explicit_recovery():
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        first = await client.post(endpoint, headers={**headers, "Idempotency-Key": str(uuid.uuid4())}, json={"reason": "acceptance drill"})
        second = await client.post(endpoint, headers={**headers, "Idempotency-Key": str(uuid.uuid4())}, json={"reason": "replayed drill"})
        ...
        resume = await client.post(f"{endpoint}/resume", headers={**headers, "Idempotency-Key": str(uuid.uuid4())}, json={})
