...
@pytest.mark.asyncio
async def test_ai_gemini_reaction_and_no_buttons():
    sender_phone = f"555{str(uuid.uuid4().int)[:6]}"
    message_id = f"msg_to_react_{uuid.uuid4().hex}"
    incoming_content = "Please react to this message and reply."

    # We send structured JSON as the mock Gemini response with suggestedReaction
    mock_gemini_json = """
    {
      "intent": "inquiry",
      "sentiment": "neutral",
      ...
      "replyContent": "[Mock Gemini Reply] We got your message.",
      "confidence": 0.95,
      "suggestedReaction": "💖"
    }
    """
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        # 1. Create Project
        proj_resp = await client.post(f"{BASE_URL}/projects", json={"name": "AiReactProj"})
        assert proj_resp.status_code == 201
        proj_id = proj_resp.json()["id"]
        ...
        # 6. Wait for message aggregation and AI reply generation (around 6-8 seconds)
        print("Waiting 10 seconds for message aggregation, AI reply, and reaction...")
        await pytest.importorskip("asyncio").sleep(10.0)

        # 7. Check if the gateway received the AI reply and the reaction
        sent_resp = await client.get(f"{BASE_URL}/whatsapp/mock/sent")
        assert sent_resp.status_code == 200
        sent_messages = sent_resp.json()

        # Find the message sent to our sender_phone
        reply_message = next((m for m in sent_messages if m.get("to") == sender_phone and "message" in m), None)
        assert reply_message is not None, "AI Reply was not sent to WhatsApp Gateway"
        
        # Verify it does NOT have buttons
        assert not reply_message.get("buttons"), "AI Reply sent buttons even though they should be removed"
        assert "[Mock Gemini Reply]" in reply_message["message"]

        # Find the reaction sent to our sender_phone
        reaction_message = next((m for m in sent_messages if m.get("to") == sender_phone and "reaction" in m), None)
        assert reaction_message is not None, "AI Auto-reaction was not sent to WhatsApp Gateway"
        assert reaction_message["reaction"] == "💖"
        assert reaction_message["targetMessageId"] == message_id
