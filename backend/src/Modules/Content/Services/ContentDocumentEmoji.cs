...
internal static class ContentDocumentEmoji
{
    // Emoji property, Unicode 17.0: https://www.unicode.org/Public/17.0.0/ucd/emoji/emoji-data.txt
    ...
    internal static string Remove(string text)
    {
        ...
        while (elements.MoveNext())
        {
            ...
            if (IsEmoji(element)) { removed = true; continue; }
            if (removed && cleaned.Length > 0 && !char.IsWhiteSpace(cleaned[^1]) && !char.IsWhiteSpace(element[0]))
                cleaned.Append(' ');
    ...
    {
        if (element.Contains('\u20E3')) return true; // Keycap sequences; ordinary digits remain text.
        foreach (var rune in element.EnumerateRunes())
        {
            if (rune.Value is 0x23 or 0x2A or >= 0x30 and <= 0x39) continue;
            if (rune.Value is 0xA9 or 0xAE or 0x2122 && !element.Contains('\uFE0F')) continue;
            if (EmojiRanges.Any(range => rune.Value >= range.Start && rune.Value <= range.End)) return true;
        }
        return false;
    }
}
