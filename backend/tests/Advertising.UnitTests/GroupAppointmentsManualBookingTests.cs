...
public sealed class GroupAppointmentsManualBookingTests
{
    ...
    [Fact]
    public async Task Requester_booking_for_a_friend_creates_two_customers_and_two_bookings()
    {
        var projectId = Guid.NewGuid();
        var (_, db) = CreateController(projectId, "Owner", projectId);
        await using var ownedDb = db;
        var group = ActiveGroup(projectId, capacity: 3);
        var requester = Customer(projectId, "201011112222", "صاحب الشات");
        ...
        var bookingResult = await orchestrator.BookSuggestedPeopleAsync(new AiGroupBookingRequest
        {
            ProjectId = projectId,
            GroupId = group.Id,
            RequesterCustomerId = requester.Id,
            SuggestedPeople =
            [
        ...
        Assert.True(bookingResult.Succeeded);
        var customers = await db.Customers.AsNoTracking().OrderBy(customer => customer.PhoneNumber).ToListAsync();
        var bookings = await db.GroupAppointmentBookings.AsNoTracking().OrderBy(booking => booking.CustomerPhone).ToListAsync();
        Assert.Equal(2, customers.Count);
        Assert.Equal(2, bookings.Count);
        Assert.Equal("صاحب الشات", customers[0].Name);
        Assert.Equal("201011112222", customers[0].PhoneNumber);
        Assert.Equal("صاحب العميل", customers[1].Name);
        Assert.Equal("201033334444", customers[1].PhoneNumber);
        Assert.NotEqual(bookings[0].CustomerId, bookings[1].CustomerId);
    }
