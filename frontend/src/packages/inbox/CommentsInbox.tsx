...
export default function CommentsInbox() {
  ...
  const [reaction, setReaction] = useState<'LIKE' | 'LOVE' | null>(null);
  ...
  // Fetch conversations with channel=FacebookComment and resolve an exact report deep link.
  useEffect(() => {
      ...
      try {
        ...
        if (shouldResolveDeepLink && requestedConversationId) {
          ...
          if (resolution.conversation) {
            ...
            showToast('محادثة التعليقات المطلوبة غير موجودة أو لم تعد متاحة.', 'warning');
          }
        }
        ...
        if (controller.signal.aborted) return;
        console.error('Error loading Comment conversations', error);
        setLoadError('تعذر تحميل محادثات التعليقات. تحقق من الاتصال ثم أعد المحاولة.');
      } finally {
  ...
  const loadMoreConversations = async () => {
    ...
    try {
      ...
      if (controller.signal.aborted || conversationQueryKeyRef.current !== requestQueryKey) return;
      console.error('Error loading older comment conversations', error);
      showToast('تعذر تحميل محادثات التعليقات الأقدم.', 'error');
    } finally {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setMessageLoadError({ conversationId, message: 'تعذر تحميل رسائل التعليقات أو بيانات العميل. لم يتم عرض سجل فارغ بديلًا عنها.' });
      } finally {
  ...
  const loadOlderMessages = async () => {
    ...
    try {
      ...
      console.error('Error loading older comment messages', error);
      showToast('تعذر تحميل رسائل التعليقات الأقدم.', 'error');
    } finally {
  ...
  const handleSend = async () => {
    ...
    const sentPublicComment = publicComment.trim();
    const sentPrivateDM = privateDM.trim();
    const sentReaction = reaction;
    setSending(true);
    try {
      await api.post(`/api/projects/${projectId}/conversations/${conversationId}/comment-reply`, {
        publicComment: sentPublicComment || null,
        privateDM: sentPrivateDM || null,
    ...
    if (activeConvRef.current?.id === conversationId) {
      setPublicComment((current) => current.trim() === sentPublicComment ? '' : current);
      setPrivateDM((current) => current.trim() === sentPrivateDM ? '' : current);
      setReaction((current) => current === sentReaction ? null : current);
    }
    showToast('تم إرسال الرد بنجاح', 'success');
    try {
      const msgResp = await api.get<Message[]>(`/api/conversations/${conversationId}/messages`, {
        params: { limit: MESSAGE_PAGE_SIZE },
      });
      if (activeConvRef.current?.id === conversationId
        && messagesConversationIdRef.current === conversationId) {
      ...
      showToast('تم إرسال الرد، لكن تعذر تحديث سجل المحادثة الآن.', 'warning');
    }
  };
